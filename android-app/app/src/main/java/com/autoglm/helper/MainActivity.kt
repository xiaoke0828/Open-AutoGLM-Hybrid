...
class MainActivity : Activity() {
    ...
    private fun updateStatus() {
        ...
        if (service != null) {
            ...
            val connectionInfo = if (ipAddress != null) {
                "服务运行中\n端口: ${AutoGLMAccessibilityService.PORT}\n" +
                "本地连接: http://localhost:${AutoGLMAccessibilityService.PORT}\n" +
                "局域网连接: http://$ipAddress:${AutoGLMAccessibilityService.PORT}\n" +
                "远程连接: 请使用 Tailscale IP"
            } else {
                getString(R.string.server_running, AutoGLMAccessibilityService.PORT)
            }
            serverStatusText.text = connectionInfo
        } else {
